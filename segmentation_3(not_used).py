...
def ul(array):
    ...
    iqr = q3 - q1
    fl = q1 - (1.5*iqr)
    fh = np.inf#q3 + (1.5*iqr) # changed this
    return fl, fh
...
def prepare(img_names):
    ...
    for i in range(len(img_names)):
        ...
        im = cv2.morphologyEx(im, cv2.MORPH_OPEN, disk3, iterations=1) # disk2 is also very good
        im = cv2.dilate(im, disk2, iterations = 1) # changed this
        
        cr = cv2.connectedComponentsWithStats(np.asarray(1 - im, dtype=np.uint8))
        areas = cr[1]
        sizes = cr[-2][:,-1][1:]
        conn_comps = cr[0]
        zz = np.zeros_like(areas)
        for ii in np.arange(1,conn_comps,1)[ sizes > 20 ]:
            zz[ areas == ii ] = 1


        dct_denoised[img_names[i]] = 1 - zz
...
if __name__ == "__main__":
    ...
    dct_paths = {}
    m = args.path
    #m = "drive-download-20240708T113330Z-002"
    ...
    #ttxx = []
    #for i in dct_paths[groups[0]]:
    #    if "5_group" in i:
    #        ttxx.append(i)

    #dct_paths[groups[0]] = dct_paths[groups[0]][:2] + ttxx[:2]   
